...
* @Last modified time: 2016-03-13T16:44:17+08:00
...
describe('ldap admin password bind', function(){
  it('should return success', (done) => {
    client.bind(`cn=${config.ldap.admin.username},${config.ldap.base}`,
...
describe('ldap static password username bind', function(){
  it('should return sucess', function(done){
    client.bind(`dc=black, dc=${config.ldap.user.username},${config.ldap.base}`,
      'password', (err) => {
        expect(err).to.be.null;
        done();
    });
...
describe('ldap static password id bind', function(){
  it('should return sucess', function(done){
    client.bind(`dc=1, dc=${config.ldap.user.username},${config.ldap.base}`,
      'password', (err) => {
        expect(err).to.be.null;
        done();
    });
